// WARNING
//
// This file has been generated automatically by Xamarin Studio to store outlets and
// actions made in the UI designer. If it is removed, they will be lost.
// Manual changes to this file may not be handled correctly.
//
using MonoTouch.Foundation;
using System.CodeDom.Compiler;

namespace Golf_Games
{
	[Register ("landscape_score_view")]
	partial class landscape_score_view
	{
		[Outlet]
		MonoTouch.UIKit.UICollectionView gridHoleBottom9 { get; set; }

		[Outlet]
		MonoTouch.UIKit.UICollectionView gridHoleHandi1 { get; set; }

		[Outlet]
		MonoTouch.UIKit.UICollectionView gridHoleHandi2 { get; set; }

		[Outlet]
		MonoTouch.UIKit.UICollectionView gridHoleUpper9 { get; set; }

		[Outlet]
		MonoTouch.UIKit.UICollectionView gridPar1 { get; set; }

		[Outlet]
		MonoTouch.UIKit.UICollectionView gridPar2 { get; set; }

		[Outlet]
		MonoTouch.UIKit.UICollectionView gridPlayer1Lower { get; set; }

		[Outlet]
		MonoTouch.UIKit.UICollectionView gridPlayer1Upper { get; set; }

		[Outlet]
		MonoTouch.UIKit.UICollectionView gridPlayer2Lower { get; set; }

		[Outlet]
		MonoTouch.UIKit.UICollectionView gridPlayer2Upper { get; set; }

		[Outlet]
		MonoTouch.UIKit.UICollectionView gridPlayer3Lower { get; set; }

		[Outlet]
		MonoTouch.UIKit.UICollectionView gridPlayer3Upper { get; set; }

		[Outlet]
		MonoTouch.UIKit.UICollectionView gridPlayer4Lower { get; set; }

		[Outlet]
		MonoTouch.UIKit.UICollectionView gridPlayer4Upper { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelP1In { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelP1InBet { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelP1Out { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelP1OutBet { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelP1Total { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelP1TotalBet { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelP2In { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelP2InBet { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelP2Out { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelP2OutBet { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelP2Total { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelP2TotalBet { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelP3In { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelP3InBet { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelP3Out { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelP3OutBet { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelP3Total { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelP3TotalBet { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelP4In { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelP4InBet { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelP4Out { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelP4OutBet { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelP4Total { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelP4TotalBet { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelPar { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelParIn { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelParOut { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelParTotal { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelPlayer1 { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelPlayer2 { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelPlayer3 { get; set; }

		[Outlet]
		MonoTouch.UIKit.UILabel labelPlayer4 { get; set; }

		[Outlet]
		MonoTouch.UIKit.UIScrollView scrollView { get; set; }

		[Outlet]
		MonoTouch.UIKit.UIView viewBottom9 { get; set; }

		[Outlet]
		MonoTouch.UIKit.UIView viewUpper9 { get; set; }
		
		void ReleaseDesignerOutlets ()
		{
			if (gridHoleBottom9 != null) {
				gridHoleBottom9.Dispose ();
				gridHoleBottom9 = null;
			}

			if (gridHoleHandi1 != null) {
				gridHoleHandi1.Dispose ();
				gridHoleHandi1 = null;
			}

			if (gridHoleHandi2 != null) {
				gridHoleHandi2.Dispose ();
				gridHoleHandi2 = null;
			}

			if (gridHoleUpper9 != null) {
				gridHoleUpper9.Dispose ();
				gridHoleUpper9 = null;
			}

			if (gridPar1 != null) {
				gridPar1.Dispose ();
				gridPar1 = null;
			}

			if (gridPar2 != null) {
				gridPar2.Dispose ();
				gridPar2 = null;
			}

			if (gridPlayer1Lower != null) {
				gridPlayer1Lower.Dispose ();
				gridPlayer1Lower = null;
			}

			if (gridPlayer1Upper != null) {
				gridPlayer1Upper.Dispose ();
				gridPlayer1Upper = null;
			}

			if (gridPlayer2Lower != null) {
				gridPlayer2Lower.Dispose ();
				gridPlayer2Lower = null;
			}

			if (gridPlayer2Upper != null) {
				gridPlayer2Upper.Dispose ();
				gridPlayer2Upper = null;
			}

			if (gridPlayer3Lower != null) {
				gridPlayer3Lower.Dispose ();
				gridPlayer3Lower = null;
			}

			if (gridPlayer3Upper != null) {
				gridPlayer3Upper.Dispose ();
				gridPlayer3Upper = null;
			}

			if (gridPlayer4Lower != null) {
				gridPlayer4Lower.Dispose ();
				gridPlayer4Lower = null;
			}

			if (gridPlayer4Upper != null) {
				gridPlayer4Upper.Dispose ();
				gridPlayer4Upper = null;
			}

			if (labelP1In != null) {
				labelP1In.Dispose ();
				labelP1In = null;
			}

			if (labelP1InBet != null) {
				labelP1InBet.Dispose ();
				labelP1InBet = null;
			}

			if (labelP1Out != null) {
				labelP1Out.Dispose ();
				labelP1Out = null;
			}

			if (labelP1OutBet != null) {
				labelP1OutBet.Dispose ();
				labelP1OutBet = null;
			}

			if (labelP1Total != null) {
				labelP1Total.Dispose ();
				labelP1Total = null;
			}

			if (labelP1TotalBet != null) {
				labelP1TotalBet.Dispose ();
				labelP1TotalBet = null;
			}

			if (labelP2In != null) {
				labelP2In.Dispose ();
				labelP2In = null;
			}

			if (labelP2InBet != null) {
				labelP2InBet.Dispose ();
				labelP2InBet = null;
			}

			if (labelP2Out != null) {
				labelP2Out.Dispose ();
				labelP2Out = null;
			}

			if (labelP2OutBet != null) {
				labelP2OutBet.Dispose ();
				labelP2OutBet = null;
			}

			if (labelP2Total != null) {
				labelP2Total.Dispose ();
				labelP2Total = null;
			}

			if (labelP2TotalBet != null) {
				labelP2TotalBet.Dispose ();
				labelP2TotalBet = null;
			}

			if (labelP3In != null) {
				labelP3In.Dispose ();
				labelP3In = null;
			}

			if (labelP3InBet != null) {
				labelP3InBet.Dispose ();
				labelP3InBet = null;
			}

			if (labelP3Out != null) {
				labelP3Out.Dispose ();
				labelP3Out = null;
			}

			if (labelP3OutBet != null) {
				labelP3OutBet.Dispose ();
				labelP3OutBet = null;
			}

			if (labelP3Total != null) {
				labelP3Total.Dispose ();
				labelP3Total = null;
			}

			if (labelP3TotalBet != null) {
				labelP3TotalBet.Dispose ();
				labelP3TotalBet = null;
			}

			if (labelP4In != null) {
				labelP4In.Dispose ();
				labelP4In = null;
			}

			if (labelP4InBet != null) {
				labelP4InBet.Dispose ();
				labelP4InBet = null;
			}

			if (labelP4Out != null) {
				labelP4Out.Dispose ();
				labelP4Out = null;
			}

			if (labelP4OutBet != null) {
				labelP4OutBet.Dispose ();
				labelP4OutBet = null;
			}

			if (labelP4Total != null) {
				labelP4Total.Dispose ();
				labelP4Total = null;
			}

			if (labelP4TotalBet != null) {
				labelP4TotalBet.Dispose ();
				labelP4TotalBet = null;
			}

			if (labelParIn != null) {
				labelParIn.Dispose ();
				labelParIn = null;
			}

			if (labelParOut != null) {
				labelParOut.Dispose ();
				labelParOut = null;
			}

			if (labelParTotal != null) {
				labelParTotal.Dispose ();
				labelParTotal = null;
			}

			if (labelPlayer1 != null) {
				labelPlayer1.Dispose ();
				labelPlayer1 = null;
			}

			if (labelPlayer2 != null) {
				labelPlayer2.Dispose ();
				labelPlayer2 = null;
			}

			if (labelPlayer3 != null) {
				labelPlayer3.Dispose ();
				labelPlayer3 = null;
			}

			if (labelPlayer4 != null) {
				labelPlayer4.Dispose ();
				labelPlayer4 = null;
			}

			if (scrollView != null) {
				scrollView.Dispose ();
				scrollView = null;
			}

			if (viewBottom9 != null) {
				viewBottom9.Dispose ();
				viewBottom9 = null;
			}

			if (viewUpper9 != null) {
				viewUpper9.Dispose ();
				viewUpper9 = null;
			}

			if (labelPar != null) {
				labelPar.Dispose ();
				labelPar = null;
			}
		}
	}
}
